...
public class DeviceBcCameraActivity extends BaseTitleActivity {
    ...
    private String config = "{\"deviceID\":\"BC_DEVICEID\",\"data\":[{\"group\":\"bc_dev\",\"param\":[{\"key\":\"gwID\",\"type\":\"string\",\"value\":\"BC_GWID\"},{\"key\":\"devId\",\"type\":\"string\",\"value\":\"BC_DEVICEID\"},{\"key\":\"deviceId\",\"type\":\"string\",\"value\":\"BC_DEVICEID\"}],\"item\":[{\"type\":\"jump\",\"name\":\"BC_DEVICEINFO\",\"action\":\"jump:DevBc_deviceInfo\"},{\"type\":\"custom\",\"name\":\"BC_SETTINGS\",\"action\":\"custom:DevBc_settings\"},{\"type\":\"custom\",\"name\":\"BC_USERMANAGER\",\"action\":\"custom:DevBc_userManager\"},{\"type\":\"custom\",\"name\":\"BC_USERMANAGER\",\"action\":\"custom:touchid_unlock\"},{\"type\":\"custom\",\"name\":\"BC_WIFICONFIG\",\"action\":\"custom:DevBc_wifiSetting\"}]}],\"cameraId\":\"BC_CAMERAID\",\"cameraWifi\":\"BC_CAMERAWIFI\"}";

    public static void startActivity(Context context, String lockDeviceId) {
        Intent intent = new Intent(context, DeviceBcCameraActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_devicebc_camera, true);
...
//            view_video.getRenderFrame(IPCGetFrameFunctionType.FRAME_MAIN_THUNBNAIL);
    ...
    protected void onDestroy() {
        ...
        DevBcCallToAnswerActivity.isReceived = false;
...
//                    view_video.getRenderFrame(IPCGetFrameFunctionType.FRAME_PLAY_THUMBNAIL);
        ...
        if (v == iv_snapshot) {
            ...
            if (TextUtils.equals(encryptPwd, "")) {
                ...
                BiometricListener biometricListener = new BiometricListener(DeviceBcCameraActivity.this, device, pwd);
            ...
            handler.removeCallbacks(reWakeTask);
            Intent intent = new Intent(this, DeviceMoreActivityForBc.class);
...
//                .replaceAll("BC_CAMERAWIFI", wifiName);
    ...
    private void getCameraIdByCloud() {
        dataApiUnit.doGetBcCameraId(lockDeviceId, "Bc", new DataApiUnit.DataApiCommonListener<String>() {
            @Override
        ...
        if (!MainApplication.getApplication().hasInitSip) {
            IPCResultCallBack initRTCAsyncCallback = new IPCResultCallBack() {
                ...
                public void getResult(int result) {
                    ...
                    LogUtil.WriteBcLog("初始化sip:" + result);
                    if (hasInit) {
    ...
    private void startRegister() {
        IPCResultCallBack ipcResultCallBack = new IPCResultCallBack() {
            ...
            public void getResult(int i) {
                ...
                LogUtil.WriteBcLog("注册Sip账号结果: " + isRegisterAccount);
                if (isRegisterAccount) {
                    makeCall();
                    LogUtil.WriteBcLog("注册成功sip后发起呼叫");
                    handler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
...
//                        finish();
    ...
    public void onMessageEvent(IPCCallStateMsgEvent event) {
        ...
        switch (callState) {
            case STATE_ESTABLISHED:
                retryCount = 0;
                LogUtil.WriteBcLog("##建立连接了");
                handler.removeCallbacks(connectRunnable);
                handler.removeCallbacks(reWakeTask);
                LogUtil.WriteBcLog("remove awakeAndCall");
                break;
            case STATE_TERMINATED:
                LogUtil.WriteBcLog("##挂断了");
                tv_network_speed.setText("0KB/s");
    ...
    private void saveWifiInfoToCloud(String key, String value) {
        ...
        dataApiUnit.doSaveDeviceKeyValue(device.devID, key, value, new DataApiUnit.DataApiCommonListener<Object>() {
            @Override
    ...
    public void onGatewayConfigEvent(GatewayConfigEvent event) {
        boolean isRight = event != null
                && event.bean != null
                && TextUtils.equals(event.bean.d, device.devID)
            ...
            if (event.bean.m == 3) {
                ...
                if (!StringUtil.isNullOrEmpty(event.bean.v)) {
                    ...
                    try {
                        ...
                        currentWifiName = jsonObject.optString("wifiName");
                        cameraId521 = jsonObject.optString("cameraId");
                        if (!TextUtils.isEmpty(currentWifiName) && !TextUtils.isEmpty(cameraId521)) {
                            getSipInfo();
                            //如果从云没查到绑定关系，从521查到已配WiFi,主动保存信息到云
                            saveWifiInfoToCloud("wifiName", currentWifiName);
                            saveWifiInfoToCloud("bindResult", "1");
                            WLog.i(PROCESS, "保存521查询的bc锁WiFi信息到云");
                        } else {
                            if (!TextUtils.equals("1", bindResult) || TextUtils.isEmpty(currentWifiName)) {
                                showConfigDialog();
                            }
                        }
                    } catch (JSONException e) {
                    ...
                    if (!TextUtils.equals("1", bindResult) || TextUtils.isEmpty(wifiName)) {
                        showConfigDialog();
                    }
                }
            }
