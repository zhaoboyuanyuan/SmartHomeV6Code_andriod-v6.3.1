...
public class HomeWidget_Cm_Switch extends FrameLayout implements IWidgetLifeCycle, View.OnClickListener {
    ...
    public HomeWidget_Cm_Switch(@NonNull Context context) {
        super(context);
        mContext = context;
        initView(context);
    }

    public HomeWidget_Cm_Switch(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        initView(context);
    }
    ...
    private void initView(Context context) {
        final View rootView = LayoutInflater.from(context).inflate(R.layout.view_home_bv_switch, null);
        ...
        if (mode == 2) {
            ...
            switch (type) {
                case SWITCH_TYPE:
                    ...
                    iv_icon_temp.setImageResource(R.drawable.icon_switch_mode_off);
                    break;
                case BIND_TYPE:
                    tv_name_temp.setText(R.string.modeBind);
                    iv_icon_temp.setImageResource(R.drawable.icon_switch_bind_mode_offline);
                    break;
                case SCENE_TYPE:
                    tv_name_temp.setText(R.string.modeScene);
                    iv_icon_temp.setImageResource(R.drawable.icon_switch_scene_mode_offline);
                    break;
            }
        } else {
            //设置title上线状态
                ...
                case SWITCH_TYPE:
                    switch (iconType) {
                        case "0":
                            ...
                            break;
                        case "1":
                            iv_icon_temp.setImageResource(R.drawable.icon_switch_mode_on);
                            break;
                        default:
                            iv_icon_temp.setImageResource(R.drawable.icon_switch_mode_off);
                            break;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.one_switch_image:
                ...
                if (!BIND_DEVICE.equals(one_type) && !BIND_SCENE.equals(one_type)) {
                    iv_one_switch.setEnabled(false);
            ...
            case R.id.two_switch_image:
                ...
                if (!BIND_DEVICE.equals(two_type) && !BIND_SCENE.equals(two_type)) {
                    iv_two_switch.setEnabled(false);
