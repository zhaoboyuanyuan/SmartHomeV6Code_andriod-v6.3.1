		...
		if(item[i].type == "search" && item[i].placeholder != "" && item[i].localName == "input") {
			...
			if(languagerText) {
				if(item[i].type == "button") {
					...
					if (item[i].getAttribute("key") == 'device_Bb_NotBound') {
 ...
 */
function globalLang() {
	window.noBinding = languageUtil.getResource("device_Bb_Unbound");
	window.noBind = languageUtil.getResource("noBind");
	window.bindTip = languageUtil.getResource("bindTip");
	window.finished = languageUtil.getResource("finished");
	window.deviceDefaultName = languageUtil.getResource("device_type_37_name");
