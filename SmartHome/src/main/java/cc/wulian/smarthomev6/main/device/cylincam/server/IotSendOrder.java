package cc.wulian.smarthomev6.main.device.cylincam.server;
...
import cc.wulian.smarthomev6.main.device.cylincam.bean.CylincamProtectBean;
...
public class IotSendOrder {
    ...
    //add syf
    //获取桌面摄像机历史录像
    public static void sendIoctrlGetPlayBackFile(TKCamHelper mCamera,byte[]temp, byte[] start, byte[] end) {
        mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL,
                AVIOCTRLDEFs.IOTYPE_USER_IPCAM_SEARCH_PLAYBACK_FILE_REQ,
                AVIOCTRLDEFs.SMsgAVIoctrlGetPlayBackFileReq.parseContent(temp,start, end));
    }
    ...
    //设置小物摄像机安全防护
    public static void sendMotionDetection(TKCamHelper mCamera, CylincamProtectBean bean) {
        if (mCamera != null) {
            mCamera.sendIOCtrl(Camera.DEFAULT_AV_CHANNEL, AVIOCTRLDEFs.IOTYPE_USER_IPCAM_SETMOTIONDETECT_REQ,
                    AVIOCTRLDEFs.SMsgAVIoctrlSetMotionDetectReq.parseContent(bean.getSwitching(), bean.getSensitivity(), bean.getArea(), bean.getDefenceused(), bean.getWeek(),
                            bean.getMoveTime()));
        }
    }
