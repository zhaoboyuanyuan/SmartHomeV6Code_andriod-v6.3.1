...
(function($, window, document, undefined) {
    ...
    var MyPicker = $.MyPicker = function(holder, options) {
        var self = this;
        self.holder = holder;
    ...
    MyPicker.prototype.calcElementItemPostion = function(andGenerateItms) {
        var self = this;
        if (andGenerateItms) {
            self.items = [];
        }
    ...
    MyPicker.prototype.calcAngle = function(c) {
        var self = this;
    ...
    MyPicker.prototype.calcElementItemVisibility = function(angle) {
        var self = this;
        self.elementItems.forEach(function(item) {
    ...
    MyPicker.prototype.setAngle = function(angle) {
        var self = this;
    ...
    MyPicker.prototype.bindEvent = function() {
        var self = this;
        var lastAngle = 0;
        var startY = null;
        var isPicking = false;
    ...
    MyPicker.prototype.initInertiaParams = function() {
        var self = this;
        self.lastMoveTime = 0;
    ...
    MyPicker.prototype.updateInertiaParams = function(event, isStart) {
        var self = this;
    ...
    MyPicker.prototype.startInertiaScroll = function(event) {
        isNeedHandleTemp = true;
        var self = this;
        var point = event.changedTouches ? event.changedTouches[0] : event;
        /**
         * 缓动代码
    ...
    MyPicker.prototype.scrollDistAngle = function(nowTime, startAngle, distAngle, duration) {
        var self = this;
        self.stopInertiaMove = false;
    ...
    MyPicker.prototype.quartEaseOut = function(t, b, c, d) {
        return -c * ((t = t / d - 1) * t * t * t - 1) + b;
    };

    MyPicker.prototype.endScroll = function() {
        var self = this;
    ...
    MyPicker.prototype.triggerChange = function(force) {
        ...
        setTimeout(function() {
            ...
            var liNum = $(".mui-pciker-list")[0].getElementsByTagName("li").length
            if(liNum){
                for (var liIndex = 0;liIndex < liNum;liIndex++){
                    var liItem = $(".mui-pciker-list")[0].getElementsByTagName("li")[liIndex];
                }
            }
            $(".my-mui-pciker-list li").each(function () {
                this.style.fontSize = "1.5rem";
            })
            var highlightItem = $(".my-mui-pciker-list .highlight")
            if (highlightItem == undefined){
                return;
            }
            $(".my-mui-pciker-list .highlight")[0].style.fontSize = "10.0rem"
            $(".my-mui-pciker-list .highlight")[0].style.lineHeight = "10.0rem"
            $(".my-mui-pciker-list .highlight")[0].style.height = "10.0rem"
            // $(".my-mui-pciker-list .highlight").css("font-size", "10.0rem");
            // $(".my-mui-pciker-list .highlight").css("line-height", "10.0rem");
            // $(".my-mui-pciker-list .highlight").css("height", "10.0rem");
            self.lastIndex = index;
            typeof force === 'function' && force();
            shownum = index + MinTemperature;
            //发送设置的温度
            if (deviceSetTemperature && isCanChangeNum){
                isNeedHandleTemp = false;
                if (shownum != deviceSetTemperature){
                    //比对温度
                    if (deviceState == "1"){
                        setTemperature(shownum);
                    }
                }
            }
        }, 0);
    ...
    MyPicker.prototype.correctAngle = function(angle) {
        var self = this;
    ...
    MyPicker.prototype.setItems = function(items) {
        var self = this;
        self.items = items || [];
        var buffer = [];
    ...
    MyPicker.prototype.getItems = function() {
        var self = this;
        return self.items;
    };

    MyPicker.prototype.getSelectedIndex = function() {
        var self = this;
        return parseInt((self.list.angle / self.itemAngle).toFixed(0));
    };

    MyPicker.prototype.setSelectedIndex = function(index, duration, callback) {
        var self = this;
        self.list.style.webkitTransition = '';
    ...
    MyPicker.prototype.getSelectedItem = function() {
        var self = this;
        return self.items[self.getSelectedIndex()];
    };

    MyPicker.prototype.getSelectedValue = function() {
        var self = this;
        return (self.items[self.getSelectedIndex()] || {}).value;
    };

    MyPicker.prototype.getSelectedText = function() {
        var self = this;
        return (self.items[self.getSelectedIndex()] || {}).text;
    };

    MyPicker.prototype.setSelectedValue = function(value, duration, callback) {
        var self = this;
    ...
    if ($.fn) {
        $.fn.createPicker = function(options) {
            //遍历选择的元素
            this.each(function(i, element) {
                if (element.picker) return;
                if (options) {
                    element.picker = new MyPicker(element, options);
                } else {
                    var optionsText = element.getAttribute('data-picker-options');
                    var _options = optionsText ? JSON.parse(optionsText) : {};
                    element.picker = new MyPicker(element, _options);
                }
            });
...
(function($, document) {
    ...
    var panelBuffer = '<div class="my-mui-poppicker">\
		<div class="mui-poppicker-header my-mui-poppicker-header">\
			<button class="mui-btn mui-poppicker-btn-cancel">取消</button>\
			<button class="mui-btn mui-btn-blue mui-poppicker-btn-ok">确定</button>\
			<div class="mui-poppicker-clear"></div>\
		</div>\
		<div class="mui-poppicker-body my-mui-poppicker-body">\
		</div>\
	</div>';

    var pickerBuffer = '<div class="mui-picker my-mui-picker ">\
		<div class="mui-picker-inner my-mui-picker-inner">\
			<div class="mui-pciker-rule mui-pciker-rule-ft my-mui-pciker-rule-ft"></div>\
			<ul class="mui-pciker-list my-mui-pciker-list">\
			</ul>\
    ...
    //定义弹出选择器类
    var MyPopPicker = $.MyPopPicker = $.Class.extend({
        //构造函数
        init: function(options) {
            ...
            document.getElementById(self.options.parentId).appendChild(self.panel);
        ...
        _createPicker: function() {
            ...
            for (var i = 1; i <= layer; i++) {
                ...
                var picker = $(pickerElement).createPicker();
