...
//    private String toastText;
    ...
    public HomeWidget_77_MeteringSocket(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        initView(context);
    }
    ...
    private void initView(Context context) {
        final View rootView = LayoutInflater.from(context).inflate(R.layout.view_home_77_metering_socket, null);
    ...
    private void updatePowerInfo(String value) {
        if (value != null && value.length() >= 10) {
            if (openState == 1) {//开的状态才显示数值
                try {
                    String powerValueText = value.substring(0, 4);
            ...
            try {
                String quantityValueText = value.substring(4, 10);
                    ...
                    EndpointParser.parse(event.device, new EndpointParser.ParserCallback() {
                        ...
                        public void onFindAttribute(Endpoint endpoint, Cluster cluster, Attribute attribute) {
                            if (cluster.clusterId == 6) {
                                if (attribute.attributeId == 0) {
                                    switchState = attribute.attributeValue;
                                } else if (attribute.attributeId == 0x8001) {
                                    powerInfo = attribute.attributeValue;
                                }
                            } else if (cluster.clusterId == 0x0b04) {
