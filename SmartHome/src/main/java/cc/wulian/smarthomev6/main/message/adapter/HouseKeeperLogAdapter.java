...
import cc.wulian.smarthomev6.entity.HouseKeeperMessageNewInfo;
...
public class HouseKeeperLogAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    public static final int TYPE_FIRST = 0;
    public static final int TYPE_NORMAL = 1;
    public static final int TYPE_NORMAL_ICON = 2;
    private List<HouseKeeperMessageNewInfo> mData;

    public HouseKeeperLogAdapter() {
        mData = new ArrayList<>();
    }

    public void addNew(List<HouseKeeperLogBean.HouseKeeperData> data) {
        if (data.isEmpty()) {
            return;
        }
        if (mData.isEmpty()) {
        ...
        List<HouseKeeperMessageNewInfo> list = new ArrayList<>();
        for (HouseKeeperLogBean.HouseKeeperData bean : data) {
            list.add(HouseKeeperMessageNewInfo.getMessageBean(bean));
            WLog.i("HouseKeeperLogAdapter", "Log: " + HouseKeeperMessageNewInfo.getMessageBean(bean).getMsg());
        }

        list.get(0).setDayFirst(true);
        list.add(0, HouseKeeperMessageNewInfo.getDateBean(list.get(0).getRecordListBean()));
        // 如果最新一条消息与 第一条消息是同一天
        ...
        for (int i = 1, count = list.size(); i < count; i++) {
            ...
            if (!list.get(i).isSameDay(list.get(i - 1))) {
                ...
                list.add(i, HouseKeeperMessageNewInfo.getDateBean(list.get(i).getRecordListBean()));
                count++;
                i++;
            }
    ...
    public void addOld(List<HouseKeeperLogBean.HouseKeeperData> data) {
        if (data.isEmpty()) {
            return;
        }
        List<HouseKeeperMessageNewInfo> list = new ArrayList<>();
//        sort(data);
        for (HouseKeeperLogBean.HouseKeeperData bean : data) {
            list.add(HouseKeeperMessageNewInfo.getMessageBean(bean));
            WLog.i("MessageLogActivity", "Log: " + HouseKeeperMessageNewInfo.getMessageBean(bean).getFullDate());
        }
    ...
    public void setData(List<HouseKeeperLogBean.HouseKeeperData> data) {
        if (data.isEmpty()) {
            return;
        }
        List<HouseKeeperMessageNewInfo> list = new ArrayList<>();
//        sort(data);
        ...
        list.get(0).setDayFirst(true);
        HouseKeeperLogBean.HouseKeeperData sceneData = list.get(0).getRecordListBean();
        list.add(0, HouseKeeperMessageNewInfo.getDateBean(sceneData));
        list.get(list.size() - 1).setDayLast(true);
        for (int i = 1, count = list.size(); i < count; i++) {
            if (list.get(i).isDate()) {
                continue;
        ...
        mData = list;
        for (HouseKeeperMessageNewInfo b : list) {
    ...
    public void sort(List<HouseKeeperLogBean.HouseKeeperData> data) {
        if (data == null) {
            return;
        }
        Collections.sort(data, new Comparator<HouseKeeperLogBean.HouseKeeperData>() {
            @Override
            public int compare(HouseKeeperLogBean.HouseKeeperData o1, HouseKeeperLogBean.HouseKeeperData o2) {
