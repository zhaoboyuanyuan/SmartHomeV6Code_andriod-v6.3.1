...
public class WaveView extends RenderView {
    ...
    private int regionStartColor = getResources().getColor(R.color.colorPrimary);
    private int regionCenterColor = getResources().getColor(R.color.white);
    private int regionEndColor = 0xff222222;
...
//        getHolder().setFormat(PixelFormat.TRANSLUCENT);
    ...
    private int test_gap = 0;

    public void setAimAmplitude(float aim_amplitude) {
        this.aim_amplitude = aim_amplitude;
    }
...
//            amplitude = width >> 3;//振幅为宽度的1/8
            min_amplitude = 10;
            max_amplitude = height * 0.45f;
            amplitude = min_amplitude;
            aim_amplitude = amplitude;
...
//        canvas.drawColor(0xff222222);
        ...
        for (int i = 2; i < crestAndCrossCount; i += 2) {
            ...
            paint.setShader(new LinearGradient(0, centerHeight + crestY, 0, centerHeight - crestY, regionStartColor, regionEndColor, Shader.TileMode.CLAMP));
...
//        if (test_gap > 100) {
//            test_gap = 0;
//            setAimAmplitude(new Random().nextInt((int) getMax_amplitude()));
//        }
    }
