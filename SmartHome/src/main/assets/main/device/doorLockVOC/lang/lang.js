function initlan() {
	try {
		...
		if(iStr == "iphoneX") {
            ...
            var lockBack = document.getElementsByClassName("lockbackimageview")
			if(offArr != '' && offArr.length != 0){
				offArr[0].style.top = "8.8rem";
                offArr[0].style.height = "calc(100% - 8.8rem)";
                lockBack[0].style.height = "calc(100% - 8.8rem)";
			}
...
function globalLang() {
	...
	window.lang_DoorLookFailed       	= languageUtil.getResource("DoorLookFailed");
    ...
    window.lang_DoorLookInsure     	= languageUtil.getResource("DoorLookInsure");
    window.lang_DoorLookRemoveInsure     	= languageUtil.getResource("DoorLookRemoveInsure");
    window.lang_DoorLookRemoveLock     	= languageUtil.getResource("DoorLookRemoveLock");
    window.lang_DoorLookLocked     	= languageUtil.getResource("DoorLookLocked");
    ...
    window.lang_DoorLookForcedLock     	= languageUtil.getResource("DoorLookForcedLock");
    window.lang_DoorLookAutoLock    	= languageUtil.getResource("DoorLookAutoLock");
    window.lang_DoorLookRegisterPWD    	= languageUtil.getResource("DoorLookRegisterPWD");
    window.lang_Alarm = languageUtil.getResource("Alarm")
    window.lang_Log = languageUtil.getResource("Log")
    window.device_70_fingerprint = languageUtil.getResource("device_70_fingerprint")
    window.device_70_card = languageUtil.getResource("device_70_card")
    window.device_70_User_Management = languageUtil.getResource("device_70_User_Management")
    window.device_70_delete = languageUtil.getResource("device_70_delete")
}
