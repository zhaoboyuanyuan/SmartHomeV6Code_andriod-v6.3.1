(function() {
	...
	var tools = {
		...
		selectPhotoForCus: function(onsucceed) {
			var callback = function(bridge) {
				bridge.callHandler("selectPhotoForCus",onsucceed);
			}
			return B.exec(callback, _BARCODE, "selectPhotoForCus");
		},
		takePhotoForCus: function(onsucceed) {
			var callback = function(bridge) {
				bridge.callHandler("takePhotoForCus",onsucceed);
			}
			return B.exec(callback, _BARCODE, "takePhotoForCus");
		},
		uploadChatPic: function(param,onsucceed) {
			var callback = function(bridge) {
				bridge.callHandler("uploadChatPic",param,onsucceed);
			}
			return B.exec(callback, _BARCODE, "uploadChatPic");
		},
	};
