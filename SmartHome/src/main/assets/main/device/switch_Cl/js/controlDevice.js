...
function reloadUI(i, status, name) {
    ...
    if (status == "0" || status == "") {  //关闭
        ...
        if (isNull(bindSceneID1) && isNull(bindSceneID2)){
            ...
            if (!isNull(bindSceneID1) && !isNull(bindSceneID2)) {
                var name1 = bindDataDicTemp.sceneName.length>4 ? bindDataDicTemp.sceneName.substr(0, 4)+'..' : bindDataDicTemp.sceneName
                var name2 = bindDataDicTemp.sceneName_D.length>4 ? bindDataDicTemp.sceneName_D.substr(0, 4)+'..' : bindDataDicTemp.sceneName_D
                name = name1 + ' / ' + name2
            } else {
...
function doClick(self, gwID, devID, endpointNumber, clusterId, statusStr) {
    ...
    if ($(self).attr("data-state") == "bind") {
        ...
        showDialog.show(switch_Cl_NoSwitchMode, 3000)
