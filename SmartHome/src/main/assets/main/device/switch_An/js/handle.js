        ...
        if(data.cmd == "500"/* && data.devID == devID*/){
            ...
            if(data.retCode === 0 || !data.retCode){
                ...
                if(data.endpointName){
                    window.showDialog.show("子设备名称重复",3000);
                }
            }
...
function updateMode() {
    ...
    $("#modeName2").html(getModeName(switchMode[2]));
}

function updateImg() {
    // todo 后续这里 id 改掉
    for (var i = 1; i <= 2; i++) {
        $('#switch' + i).attr('class', 'switchImage');
        switch (switchMode[i]) {
            case 0x0002:
...
function initEvent() {
    ...
    $("#modeName1").on("click", function () {
        ...
        if(G_deviceIsShare == false) {
            window.location="../switch_An/changemode.html?devID="+devID
                +"&epNum="+1
                +"&epMode="+switchMode[1];
        } else {
            window.showDialog.show(Share_No_Permission,3000);
