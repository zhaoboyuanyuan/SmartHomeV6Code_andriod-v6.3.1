...
plus.plusReady(function () {
    plus.gatewayCmd.getDeviceInfo(function (result) {
        ...
        gwID = result.gwID
        // 获取更多配置
        moreConfig = getMoreConfig(dID);
        //设备名称
        var name = result.name.indexOf("#$default$#") != -1 ? lang_name_op + result.name.split("#")[2] : result.name
    ...
    plus.gatewayCmd.newDataRefresh(function (data) {
        if (data.cmd == "500" && data.devID == dID) {
            var name = data.name.indexOf("#$default$#") != -1 ? lang_name_op + data.name.split("#")[2] : data.name
            $(".deviceName").html(name)
            deviceMode(data)
            isSendCmd = false;
...
$(".btn-i span").find("i").on("click", function () {
    ...
    if ($(this).parent().index() == 9) {
        if (couldDelete) {
            ...
            $(".remove").css({
                "background": "url(fonts/backspace.png) no-repeat center center",
        ...
        if (str.length == 0) {
            ...
            $(".remove").css({
                "background": "url(fonts/backspace_pre.png) no-repeat center center",
                "background-size": "3.15rem auto"
            })
            $(".removeAll i").css({
                "color": "#8dd652"
        ...
        case 32769:
            {
                ...
                if (attributeValue == 4) {
                    ...
                    $(".lock_img").css({
                        "background": "url(fonts/icon_doorLock_on.png) no-repeat center center",
