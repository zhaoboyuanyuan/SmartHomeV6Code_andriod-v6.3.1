...
public class AtRecoverStateView extends FrameLayout implements IDeviceMore {
    private static String TAG = AtRecoverStateView.class.getSimpleName();
    ...
    public AtRecoverStateView(Context context) {
        super(context);
            ...
            if (TextUtils.equals(event.device.devID, deviceID)) {
                if (event.device.mode == 3) {
                    ...
                    for (int i = 0; i < clusters.size(); i++) {
                        ...
                        if (cluster.getInteger("clusterId") == 0x0006) {//开关信息
                            JSONArray attributes = cluster.getJSONArray("attributes");
                            for (int j = 0; j < attributes.size(); j++) {
                                JSONObject attribute = attributes.getJSONObject(j);
                                if (attribute.getInteger("attributeId") == 0x8006) {
                                    try {
