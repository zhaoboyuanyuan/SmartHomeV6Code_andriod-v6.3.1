...
public class RegulateSwitch extends View {
    ...
    public RegulateSwitch(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RegulateSwitch(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
...
//            WLog.i("luzx", "sweepAngle:" + sweepAngle);
            ...
            if (percentTemp == 0) {
                ...
                String text = null;
                if (showBrightness) {
                    text = getResources().getString(R.string.Device_91_Brightness) + String.format("%.0f", percent * waitingPercent * 100);
                    mPaint.setTextSize(textSize * 0.3f);
                } else {
                    text = String.format("%.0f", percent * waitingPercent * 100);
...
//                Log.i("luzx", "percent:" + percent);
                    ...
                    if (showBrightness) {
                        mPaint.setTextSize(textSize * 0.3f);
                        text = getResources().getString(R.string.Device_91_Brightness) + String.format("%.0f", animationPercent * 100);
                    } else {
                        mPaint.setTextSize(textSize);
                        text = String.format("%.0f", animationPercent * 100);
                    }
...
//        canvas.drawLine(boundaryX1, boundaryY1, boundaryX2, boundaryY2, mPaint);
    ...
    public void setShowBrightness(boolean showBrightness) {
        ...
        if (oPenCloseAnimation == null) {
            ...
            oPenCloseAnimation.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    Log.i("regulateSwitch", "onAnimationUpdate: " + oPenClosePercent);
                    postInvalidate();
                }
            });
        }
