 ...
 */
function globalLang() {
    window.device_Bw_name = languageUtil.getResource("device_Cl_name")
    window.device_Bw_on = languageUtil.getResource("device_Bu_on")
    window.device_Bw_off = languageUtil.getResource("device_Bu_off")
    window.device_Bw_control = languageUtil.getResource("device_Bu_control")
    window.device_Bw_Rename = languageUtil.getResource("device_Bu_Rename")
    window.device_Bw_enter_name = languageUtil.getResource("device_Bu_enter_name")
    window.device_Bw_Cancel = languageUtil.getResource("device_Bu_Cancel")
    window.device_Bw_Confirm = languageUtil.getResource("device_Bu_Confirm")
    window.lang_unBind = languageUtil.getResource("device_Bu_Untied") //解绑
    window.lang_time_out = languageUtil.getResource("device_Bu_time_out") //连接超时
    window.lang_Untied_success = languageUtil.getResource("device_Bu_Untied_success") //解绑成功
    window.lang_Binding_success = languageUtil.getResource("device_Bu_Binding_success");//绑定成功
    window.lang_device_switch = languageUtil.getResource("device_switch"); //开关
    window.lang_offLine = languageUtil.getResource("offLine"); //离线
    window.lang_onLine = languageUtil.getResource("onLine"); //在线
    window.lang_group_text = languageUtil.getResource("group_text");
    window.lang_Log = languageUtil.getResource("Log"); //日志
    window.lang_group = languageUtil.getResource("group_text"); //未分区

    window.lang_device_rename = languageUtil.getResource("device_Bu_Rename"); // 重命名
    window.lang_device_input_name = languageUtil.getResource("device_Bu_enter_name"); //请输入名称
    window.lang_device_cancal = languageUtil.getResource("device_Bu_Cancel"); //取消
    window.lang_device_sure = languageUtil.getResource("device_Bu_Confirm"); //确定
	...
	window.switch_Cl_NoSwitchMode = languageUtil.getResource("switch_Cl_NoSwitchMode") // 非开关模式，请至设备端控制
    window.device_Bw_Binding = languageUtil.getResource("device_Bu_Binding") // 正在绑定，请稍后...
    window.device_Bw_Binding_failure = languageUtil.getResource("device_Bu_Binding_failure") // 绑定失败，请重试
    window.switch_Cl_Saving = languageUtil.getResource("switch_Cl_Saving") //正在保存
    ...
    window.switch_Cl_ChangeMode02 = languageUtil.getResource("switch_Cl_ChangeMode02") //切换后原有已绑定设备将解除绑定
	window.Share_No_Permission = languageUtil.getResource("Share_No_Permission") //分享的设备，不支持此操作
