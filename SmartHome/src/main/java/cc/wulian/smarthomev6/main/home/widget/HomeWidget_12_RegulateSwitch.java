 ...
 * Function: 单火线单路触摸调光开关 首页widget
 */

public class HomeWidget_12_RegulateSwitch extends RelativeLayout implements IWidgetLifeCycle, View.OnClickListener {

    private static final String TAG = HomeWidget_12_RegulateSwitch.class.getSimpleName();
    private Context mContext;
    private TextView tv_name, tv_sate, tv_room;
    private RegulateSwitch regulate_switch;
    private Device mDevice;

    public HomeWidget_12_RegulateSwitch(Context context) {
        super(context);
        mContext = context;
        initView(context);
    }

    public HomeWidget_12_RegulateSwitch(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        initView(context);
    }
    ...
    private void initView(Context context) {
        final View rootView = LayoutInflater.from(context).inflate(R.layout.view_home_12_regulate_switch, null);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.root_view:
                RegulateSwitchActivity.start(mContext, mDevice.devID);
                break;
        }
    }
