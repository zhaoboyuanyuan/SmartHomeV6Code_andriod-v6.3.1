 ...
 */
public class ColorTempSwitch extends View {
    ...
    private int cirqueGroundColor = 0xffff2a34;
    private int onffColor = 0xff9e9e9e;
    private int leftColor = 0xff303be7;
    private int midColor = 0xffe72ec6;
    private int rightColor = 0xffff2a34;
    ...
    public ColorTempSwitch(Context context) {
        super(context);
    }

    public ColorTempSwitch(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ColorTempSwitch(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mSweepGradient == null) {
            mSweepGradient = new SweepGradient(width / 2, height / 2, new int[]{0XFFEA0972, 0XFF8068EC, 0XFFE939DA}, null);
        }
        ...
        if (isWaiting) {
            ...
            if (mSweepGradient == null) {
                mSweepGradient = new SweepGradient(width / 2, height / 2, new int[]{rightColor, leftColor, midColor}, null);
            }
            mPaint.setShader(mSweepGradient);
            //画进度环
//            WLog.i("luzx", "sweepAngle:" + sweepAngle);
            ...
            if (percent != 0) {
                ...
                if (oPenClosePercent == 1 || lastPercent > 0) {
                    mPaint.setTypeface(Typeface.DEFAULT_BOLD);
                    mPaint.setColor(onffColor);
                    canvas.drawText("", (width - onTextWidth) / 2f, height - padding + (onTextHeight / 2f), mPaint);
                    mPaint.setColor(textColor);
                    fontMetrics = mPaint.getFontMetrics();
                    mPaint.setTextSize(textSize * 0.5f);
                    float unitTextWidth = mPaint.measureText(unit);
...
//                Log.i("luzx", "percent:" + percent);
                    String text = null;
                    if (animationPercent * 100 == 50) {
                        text = getResources().getString(R.string.Device_91_Neutral) + String.format("%.0f", animationPercent * 100);
                    } else if (animationPercent * 100 < 50) {
                        text = getResources().getString(R.string.Device_91_Cool) + String.format("%.0f", animationPercent * 100);
                    } else if (animationPercent * 100 > 50) {
                        text = getResources().getString(R.string.Device_91_Warm) + String.format("%.0f", animationPercent * 100);
                    }
...
//                Log.i("luzx", "percentText:" + text);
                    ...
                    mPaint.setTypeface(Typeface.DEFAULT_BOLD);
                    mPaint.setColor(onffColor);
                    canvas.drawText("", (width - offTextWidth) / 2f, (height - offTextHeight) / 2f + (((height - cirqueWidth) / 2f - padding) * oPenClosePercent), mPaint);
                }
            } else {
                mPaint.setStyle(Paint.Style.FILL);
                mPaint.setStrokeWidth(0);
                ...
                //画状态
                if (oPenClosePercent == 1 || lastPercent == 0) {
                    canvas.drawText("", (width - textWidth) / 2f, (height - textHeight) / 2f, mPaint);
                } else {
                    canvas.drawText("", (width - onTextWidth) / 2f, height - padding + (onTextHeight / 2f) - (((height - cirqueWidth) / 2f - padding) * oPenClosePercent), mPaint);
                }
            }
        }
