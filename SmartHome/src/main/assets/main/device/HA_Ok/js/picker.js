...
(function($, window, document, undefined) {
	...
	var MyPicker = $.MyPicker = function(holder, callback, options) {
		var self = this;
		self.holder = holder;
    ...
    MyPicker.prototype.init = function() {
		...
		if (isIos) {
			self.list.style.webkitTransformOrigin = "center center " + 150 + "px";
		}
	};
    ...
    MyPicker.prototype.calcElementItemPostion = function(andGenerateItms) {
		...
		self.elementItems.forEach(function(item) {
			...
			item.style.webkitTransformOrigin = "center center -" + 150+ "px";
			item.style.webkitTransform = "translateZ(" + 150 + "px) rotateX(" + (-self.endAngle) + "deg)";
			if (andGenerateItms) {
				var dataItem = {};
    ...
    MyPicker.prototype.endScroll = function() {
		...
		self.triggerChange(self.callback);
	};
    ...
    MyPicker.prototype.triggerChange = function(force) {
		...
		setTimeout(function() {
			...
			typeof force === 'function' && force(item);
		}, 0);
	};
	...
	if ($.fn) {
		$.fn.createPicker = function(callback, options) {
			//遍历选择的元素
			this.each(function(i, element) {
				if (element.picker) return;
				if (options) {
					element.picker = new mui.MyPicker(element, callback, options);
				} else {
					var optionsText = element.getAttribute('data-picker-options');
					var _options = optionsText ? JSON.parse(optionsText) : {};
					element.picker = new mui.MyPicker(element, callback, _options);
				}
			});
...
(function($, document) {
	...
	var panelBuffer = '<div class="mui-poppicker my-poppicker">\
	...
	var pickerBuffer = '<div class="mui-picker my-mui-picker">\
		<div class="mui-picker-inner">\
			...
			<div class="mui-pciker-rule mui-pciker-rule-bg my-mui-pciker-rule-bg"></div>\
		</div>\
	</div>';

	//定义弹出选择器类
	$.MyPopPicker = $.Class.extend({
		//构造函数
		...
		_createPicker: function(callback) {
			var self = this;
			...
			for (var i = 1; i <= layer; i++) {
				...
				var picker = $(pickerElement).createPicker(callback);
