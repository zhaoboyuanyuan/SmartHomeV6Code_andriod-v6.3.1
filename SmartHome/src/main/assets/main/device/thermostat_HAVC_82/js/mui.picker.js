...
(function($, window, document, undefined) {
    ...
    Picker.prototype.bindEvent = function() {
        ...
        self.holder.addEventListener($.EVENT_START, function(event) {
            ...
            if (!(s_82_mode == "01" || s_82_mode == "02" || s_82_mode == "03")){
                return;
            }
    ...
    Picker.prototype.triggerChange = function(force) {
        ...
        setTimeout(function() {
            ...
            $(".mui-pciker-list li").each(function () {
                this.style.fontSize = "1.5rem";
            })
            var highlightItem = $(".highlight")
            if (highlightItem == undefined){
                return;
            }
            $(".highlight")[0].style.fontSize = "8.0rem"
            self.lastIndex = index;
            typeof force === 'function' && force();
            shownum = index + MinTemperature;
            //发送设置的温度
            if (isNeedHandleTemp){
//          	if ((heatTemperatureShowNum || coolTemperatureShowNum) && isNeedHandleTemp){
                isNeedHandleTemp = false;
                //比对温度
                if (s_82_mode == "01"){
                    //制热
                    if (shownum != heatTemperatureShowNum){
                    		if((shownum <= MaxTemperature && shownum >= MinTemperature) || (!MinTemperature || !MaxTemperature)){
                       		setThermostatTemperature(shownum);
                    		}
                    }
                    
                }else if(s_82_mode == "02"){
                    //制冷
                    if (shownum != coolTemperatureShowNum){
                    		if((shownum <= MaxTemperature && shownum >= MinTemperature) || (!MinTemperature || !MaxTemperature)){
                       		setThermostatTemperature(shownum);
                    		}
                    }
                    ...
                    if (selectedMode == "heat"){
                    		heatTemperatureShowNum = shownum;
                    } else if (selectedMode == "cool"){
                    		coolTemperatureShowNum = shownum;
                    }
                    if((shownum <= MaxTemperature && shownum >= MinTemperature) || (!MinTemperature || !MaxTemperature)){
                       setThermostatTemperature(heatTemperatureShowNum,coolTemperatureShowNum);
                    }
                }
