...
import cc.wulian.smarthomev6.support.core.apiunit.ErpApiUnit;
 ...
 * 发现界面的扫码
 */
public final class ExploreQRCodeActivity extends BaseTitleActivity implements SurfaceHolder.Callback, ICaptureActivity {

    private static final String TAG = ExploreQRCodeActivity.class.getSimpleName();
    ...
    public static void start(Context context, String scanType) {
        Intent intent = new Intent(context, ExploreQRCodeActivity.class);
    ...
    public void onCreate(Bundle icicle) {
        ...
        setContentView(R.layout.activity_qrcode_explore, true);
...
//                startWork();
    ...
    protected void initTitle() {
        super.initTitle();
        setToolBarTitle(getString(R.string.Find_HaveAScan));
    }
...
//        cameraType = getIntent().getStringExtra("cameraType");
    ...
    public void handleDecode(Result rawResult, Bundle bundle) {
        ...
        handleHttpApi(rawResult.getText());

    }

    ErpApiUnit erpApiUnit;
    private static final String DATA_REQUEST = "DATA_REQUEST";

    private void handleHttpApi(String text) {
        if (erpApiUnit == null) {
            erpApiUnit = new ErpApiUnit(this);
        }
        if (!TextUtils.isEmpty(text) && text.startsWith("http")) {
            ProgressDialogManager.getDialogManager().showDialog(DATA_REQUEST, this, null, null, 5000);
            erpApiUnit.doSendRequest(text, new ErpApiUnit.CommonListener() {
                @Override
                public void onSuccess(Object bean) {
                    ToastUtil.show("请求成功");
                    ProgressDialogManager.getDialogManager().dimissDialog(DATA_REQUEST, 1);
                    finish();
                }
            ...
            ToastUtil.show(R.string.Handled_Error);
        }
    }
