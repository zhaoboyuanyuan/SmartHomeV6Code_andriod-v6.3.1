...
/**
 * Created by zbl on 2017/3/22.
 * AES对称加密解密类
 */

public class AESUtil {
    private static final String CipherMode = "AES/ECB/NoPadding";//"AES/ECB/PKCS5Padding";
    private static final String CipherMode_Padding5 = "AES/ECB/PKCS5Padding";
    ...
    // /** 加密字节数据 **/
    @Deprecated
    public static byte[] encrypt(byte[] content, String password, boolean padding) {
        try {
            SecretKeySpec key = createKey(password);
            System.out.println(key);
            Cipher cipher = null;
            if (padding){
                cipher = Cipher.getInstance(CipherMode_Padding5);
            }else{
    ...
    // /** 加密(结果为16进制字符串) **/
    @Deprecated
    public static String encrypt(String content, String password, boolean padding) {
        byte[] data = null;
        try {
            data = content.getBytes("UTF-8");
        } catch (Exception e) {
        ...
        data = encrypt(data, password, padding);
        String result = byte2hex(data);
        return result;
    }
    ...
    // /** 解密字节数组 **/
    @Deprecated
    public static byte[] decrypt(byte[] content, String password, boolean padding) {
        try {
            SecretKeySpec key = createKey(password);
            Cipher cipher = null;
            if (padding){
    ...
    // /** 解密16进制的字符串为字符串 **/
    @Deprecated
    public static String decrypt(String content, String password, boolean padding) {
        byte[] data = null;
        try {
            data = hex2byte(content);
        } catch (Exception e) {
        ...
        data = decrypt(data, password, padding);
        if (data == null)
            return null;
        String result = null;
        try {
