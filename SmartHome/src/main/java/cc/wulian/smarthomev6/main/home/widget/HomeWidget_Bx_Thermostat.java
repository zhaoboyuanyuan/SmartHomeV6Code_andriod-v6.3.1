 ...
 * Function: 联排地暖温控器 首页widget
 */

public class HomeWidget_Bx_Thermostat extends RelativeLayout implements IWidgetLifeCycle, View.OnClickListener {

    private static final String TAG = HomeWidget_Bx_Thermostat.class.getSimpleName();
    ...
    public HomeWidget_Bx_Thermostat(Context context) {
        super(context);

        initView(context);
    }

    public HomeWidget_Bx_Thermostat(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    ...
    private void initView(Context context) {
        mRootView = LayoutInflater.from(context).inflate(R.layout.view_home_bx, null);
...
//                }
    ...
    private Runnable changeTempRunnable = new Runnable() {
        @Override
        public void run() {
            String temp = Integer.toHexString((int) (mTempPre * 10));
            if (temp.length() > 2) {
                temp = "0" + temp;
