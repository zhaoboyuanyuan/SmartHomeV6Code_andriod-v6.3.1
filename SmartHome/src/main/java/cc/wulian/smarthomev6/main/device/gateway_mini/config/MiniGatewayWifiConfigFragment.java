...
public class MiniGatewayWifiConfigFragment extends WLFragment implements View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    public static MiniGatewayWifiConfigFragment newInstance(String deviceId, String wifiListData, int type, String scanEntry) {
    ...
    public int layoutResID() {
        return R.layout.fragment_mini_gateway_wifi_config;
    }
    ...
    protected void initData() {
        ...
        etWifiNameMini.setText("Mini_" + deviceId.substring(deviceId.length() - 6, deviceId.length()));
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_next_step:
                if (!TextUtils.isEmpty(etWifiPasswordMini.getText())) {
                    setAllLinkInfo();
                    FragmentTransaction ft = getFragmentManager().beginTransaction();
                    if (type == 0) {
                        miniGatewayConfigResultFragment = miniGatewayConfigResultFragment.newInstance(deviceId, 0, scanEntry);
                        ft.replace(android.R.id.content, miniGatewayConfigResultFragment, MiniGatewayConfigResultFragment.class.getName());
                        ft.addToBackStack(null);
                    } else {
                        miniGatewayConfigResultFragment = miniGatewayConfigResultFragment.newInstance(deviceId, 1, scanEntry);
                        ft.add(android.R.id.content, miniGatewayConfigResultFragment, MiniGatewayConfigResultFragment.class.getName());
                    }
                    ft.commit();
                } else {
                    ToastUtil.show(getString(R.string.Minigateway_Connectnetwork_Notbeempty));
                }
                break;
            ...
            case R.id.base_img_back_fragment:
                ...
                if (type == 0) {
                    ...
                    ft2.replace(android.R.id.content, miniGatewayGuideFragment, MiniGatewayGuideFragment.class.getName());
