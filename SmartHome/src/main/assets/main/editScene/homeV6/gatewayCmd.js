(function() {
	...
	var gatewayCmd = {
        ...
        isShowGuidePage: function(param, onsucceed) {
            var callback = function(bridge) {
                if(typeof onsucceed == "function") {
                    bridge.callHandler("isShowGuidePage", param, onsucceed);
                } else {
                    console.log("fail");
                    return;
                }
            }
            return B.exec(callback, _BARCODE, "isShowGuidePage");
        },
        recommendSceneName: function(param, onsucceed) {
            var callback = function(bridge) {
                if(typeof onsucceed == "function") {
                    bridge.callHandler("recommendSceneName", param, onsucceed);
                } else {
                    console.log("fail");
                    return;
                }
            }
            return B.exec(callback, _BARCODE, "recommendSceneName");
        },
		//判断该设备是否被禁用
