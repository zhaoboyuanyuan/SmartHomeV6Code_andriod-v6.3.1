...
public class CameraHistorySeekBar extends View {
    ...
    private static final int DEFAULT_LINE_COLOR = 0xFF8dd652;// 默认画线的颜色
    private static final int DEFAULT_TEXT_COLOR = 0xFF8dd652;// 默认文字的颜色
    ...
    private static final int DEFAULT_LEFT_HAS_RECORDED_COLOR = 0xFFB9EEEE;// 淡蓝
    private static final int DEFAULT_LEFT_SELECTED_CHANGE_COLOR = 0xFFFBC1BB;// 橘红
    private static final int MID_LINE_COLOR = 0xFF00CCCC;// 天蓝
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int perUnitSecondS = mCurrentScaleType.getMins() * DEFAULT_SECONDS_PER_MIN;
        ...
        long lastTimeStamp = (long) (currentTimeStamp + (mTotalWidth - currentOffset)
                * perUnitSecondS / (perUnitSize + mLineWidth));
            ...
            for (int i = 0; i < mRecordListSize; i++) {
                ...
                if (pair.first <= lastTimeStamp
                    ...
                    if (pair.second >= lastTimeStamp) {
                        ...
                        endX = currentOffset + (pair.second - currentTimeStamp + DEFAULT_SECONDS_PER_MIN)
                                * (perUnitSize + mLineWidth)
                                / perUnitSecondS;
                    }
        ...
        while (currentOffset <= mTotalWidth) {
            ...
            int allMinsBy24Hours = (hours * DEFAULT_SECONDS_PER_MIN + mines) / mCurrentScaleType
                    .getMins();
    ...
    public void onChangeSeekBarFinalAction(long timeStamp, boolean isRecord) {
        mHistroySeekChangeListener.onChangeSeekBarFinalAction(timeStamp, isRecord);
    }
    ...
    public void onChangeSeekBarTempAction(long timeStamp) {
        mHistroySeekChangeListener.onChangeSeekBarTempAction(timeStamp);
    }
...
//                if (mFingerState == FINGER_STATE.ACTION_MOVE) {
                if (mHistroySeekChangeListener != null && mIsActionEnable) {
                    Log.d("Cam", "onChangeSeekBarFinalAction is:"
...
//                mFingerState = FINGER_STATE.ACTION_NONE;
                break;
//		case MotionEvent.ACTION_CANCEL:
//			Log.d("Cam", "ACTION_CANCEL");
//			mLastPointerCount = 0;
//			break;
            case MotionEvent.ACTION_DOWN:
                Log.d("Cam", "ACTION_DOWN");
...
//									+ mCurrentMidTimeStamp);
    ...
    public interface HistroySeekChangeListener {
        void onChangeSeekBarFinalAction(long timeStamp, boolean isRecord);

        void onActionDownMessage();

        void onChangeSeekBarTempAction(long timeStamp);
    }
    ...
    public class FingerScaleGestureListener implements
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            float currentScale = mCurrentScaleNum * (1 + (scale - 1)
                    / (2 * 2));
            if (scale > 1.0) {// 放大
