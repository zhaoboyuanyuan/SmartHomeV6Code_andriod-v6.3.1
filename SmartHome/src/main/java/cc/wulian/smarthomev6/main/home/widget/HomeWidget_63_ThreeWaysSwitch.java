...
public class HomeWidget_63_ThreeWaysSwitch extends FrameLayout implements IWidgetLifeCycle, View.OnClickListener {
    ...
    private TextView tv_one_switch_state, tv_two_switch_state, tv_three_switch_state;
    private ImageView iv_one_switch, iv_two_switch, iv_three_switch;
    private ProgressRing pr_one_switch, pr_two_switch, pr_three_switch;
    private Device mDevice;
    private TextView tv_toast;
    private boolean isOpenCommand_oneSwitch = false;
    private boolean isOpenCommand_twoSwitch = false;
    private boolean isOpenCommand_threeSwitch = false;
    ...
    public HomeWidget_63_ThreeWaysSwitch(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        initView(context);
    }
    ...
    private void initView(Context context) {
        final View rootView = LayoutInflater.from(context).inflate(R.layout.view_home_63_three_ways_switch, null);
        ...
        iv_three_switch = (ImageView) rootView.findViewById(R.id.three_switch_image);
        ...
        pr_three_switch = (ProgressRing) rootView.findViewById(R.id.three_switch_progress);
        ...
        tv_three_switch_state = (TextView) rootView.findViewById(R.id.three_switch_state);
        ...
        iv_three_switch.setOnClickListener(this);
        iv_one_switch.setEnabled(false);
        iv_two_switch.setEnabled(false);
        iv_three_switch.setEnabled(false);
    }
...
//        Log.i("luzx", "onBindViewHolder 63:" + mDevice);
        // 设置标题
        setName();
        setRoomName();
    ...
    private void updateState(int mode, int endpointNumber, String attributeValue) {
        ...
        switch (endpointNumber){
            ...
            case 2:
                ...
                break;
            case 3:
                iv_icon_temp = iv_three_switch;
                tv_state_temp = tv_three_switch_state;
                switchState = iv_icon_temp.getTag();
                if(attributeValue != null && !attributeValue.equals(switchState)){
                    pr_three_switch.setState(ProgressRing.FINISHED);
                }
                break;
        }
...
//            Log.i("luzx", "onDeviceReport 63:" + mDevice);
//            mDevice = MainApplication.getApplication().getDeviceCache().get(mDevice.devID);
//            dealDevice(mDevice);
        }
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.three_switch_image:
                Object three_type = iv_three_switch.getTag();
                iv_three_switch.setEnabled(false);
                if ("0".equals(three_type)) {
                    sendCmd(0x01, 3);//打开
                    isOpenCommand_threeSwitch = true;
                } else {
                    sendCmd(0x00, 3);//关闭
                    isOpenCommand_threeSwitch = false;
                }
                pr_three_switch.setTimeout(5000);
                pr_three_switch.setState(ProgressRing.WAITING);
                pr_three_switch.setAnimatorListener(new ProgressRing.AnimatorListener() {
                    @Override
                    public void onTimeout() {
                        iv_three_switch.setEnabled(true);
                    }
