(function() {
	...
	var gatewayCmd = {
        ...
        getGatewayBackupData: function(param, onsucceed) {
            var callback = function(bridge) {
                if(typeof onsucceed == "function") {
                    bridge.callHandler("getGatewayBackupData", param, onsucceed);
                } else {
                    console.log("fail");
                    return;
                }
            }
            return B.exec(callback, _BARCODE, "getGatewayBackupData");
        },
        backupGatewayData:function (onsucceed) {
            var callback = function(bridge) {
                bridge.callHandler("backupGatewayData", onsucceed);
            }
            return B.exec(callback, _BARCODE, "backupGatewayData");
        },
        verifyGateway: function(param, onsucceed) {
            var callback = function(bridge) {
                if(typeof onsucceed == "function") {
                    bridge.callHandler("verifyGateway", param, onsucceed);
                } else {
                    console.log("fail");
                    return;
                }
            }
            return B.exec(callback, _BARCODE, "verifyGateway");
        },
        recoveryGatewayData: function(param, onsucceed) {
			var callback = function(bridge) {
				if(typeof onsucceed == "function") {
					bridge.callHandler("recoveryGatewayData", param, onsucceed);
				} else {
					console.log("fail");
					return;
				}
			}
			return B.exec(callback, _BARCODE, "recoveryGatewayData");
    	},
