$(function() {
	...
	function makeData(data) {
		if(data.mode == "2") {
			...
			$.each(data.endpoints, function(i, endpoints) {
				...
				var statusStr;
                var endpointName = endpoints.endpointName;
                if (endpointName == '' || endpointName == undefined || endpointName == null) {
                    endpointName = device_switch + endpoints.endpointNumber;
                }
				if(status == "0") {
	...
	function doTimeout(endpointNumber, statusStr) {
		var timer = setTimeout(function() {
            ...
            $(".socket:eq(" + endpointNumber + ") span").css("color", "gray");
