		...
		if(item[i].type == "search" && item[i].placeholder != "" && item[i].localName == "input") {
			...
			if(languagerText) {
				if(item[i].type == "button") {
					...
					if (item[i].getAttribute("key") == 'Device_32_Details_Notbound') {
						item[i].innerHTML = languagerText + '<i></i>';
					} else {
 ...
 */
function globalLang() {
    window.device_name_Ax = languageUtil.getResource("device_Ax_name");
    window.Device_Ax_Switch = languageUtil.getResource("device_Ax_Switch");
    window.Device_Ax_Notbind = languageUtil.getResource("device_Ax_NotBound");
    window.lang_Log         	= languageUtil.getResource("Log");
    window.lang_Alarm         	= languageUtil.getResource("Alarm");
    window.device_rename = languageUtil.getResource("addDevice_succ_rename");
    window.device_input_name = languageUtil.getResource("renameInput");
    window.device_sure = languageUtil.getResource("addDevice_sure");
    window.device_cancal = languageUtil.getResource("searchScene_cancal");
    window.device_switch = languageUtil.getResource("device_Ax_Switch");
}
