...
public class HomeWidget_Cl_Switch extends RelativeLayout implements IWidgetLifeCycle, View.OnClickListener {

    private static final String TAG = HomeWidget_Cl_Switch.class.getSimpleName();
    ...
    public HomeWidget_Cl_Switch(Context context) {
        super(context);
        mContext = context;
        initView(context);
    }

    public HomeWidget_Cl_Switch(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        initView(context);
    }
    ...
    private void initView(Context context) {
        final View rootView = LayoutInflater.from(context).inflate(R.layout.view_home_bu_switch, null);
        ...
        if (mode == 2) {
            ...
            switch (type) {
                case SWITCH_TYPE:
                    ...
                    iv_one_switch.setImageResource(R.drawable.icon_switch_mode_off);
                    break;
                case BIND_TYPE:
                    tv_one_switch_name.setText(R.string.modeBind);
                    iv_one_switch.setImageResource(R.drawable.icon_switch_bind_mode_offline);
                    break;
                case SCENE_TYPE:
                    tv_one_switch_name.setText(R.string.modeScene);
                    iv_one_switch.setImageResource(R.drawable.icon_switch_scene_mode_offline);
                    break;
            }
        } else {
            //设置title上线状态
                ...
                case SWITCH_TYPE:
                    switch (iconType) {
                        case "0":
                            ...
                            break;
                        case "1":
                            iv_one_switch.setImageResource(R.drawable.icon_switch_mode_on);
                            break;
                        default:
                            iv_one_switch.setImageResource(R.drawable.icon_switch_mode_off);
                            break;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.one_switch_image:
                ...
                if (!BIND_DEVICE.equals(type) && !BIND_SCENE.equals(type)) {
                    iv_one_switch.setEnabled(false);
                    if ("0".equals(type)) {
